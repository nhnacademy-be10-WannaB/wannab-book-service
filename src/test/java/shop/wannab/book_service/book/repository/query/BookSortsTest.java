...
class BookSortsTest {
    ...
    void from_sortByIdDesc() {
        ...
        OrderSpecifier<?>[] orderSpecifiers = BookSorts.from(pageable).toArray(new OrderSpecifier[0]);
        assertThat(orderSpecifiers).hasSize(1);
        assertThat(orderSpecifiers[0]).hasToString("book.bookId DESC");
    }
    ...
    void from_sortByTitleAsc() {
        ...
        OrderSpecifier<?>[] orderSpecifiers = BookSorts.from(pageable).toArray(new OrderSpecifier[0]);
        assertThat(orderSpecifiers).hasSize(1);
        assertThat(orderSpecifiers[0]).hasToString("book.title ASC");
    }
    ...
    void from_sortByMultipleProperties() {
        ...
        assertThat(orderSpecifiers[0]).hasToString("book.bookId DESC");
        assertThat(orderSpecifiers[1]).hasToString("book.title ASC");
    }
    ...
    void from_noSort() {
        ...
        OrderSpecifier<?>[] orderSpecifiers = BookSorts.from(pageable).toArray(new OrderSpecifier[0]);
        assertThat(orderSpecifiers[0]).hasToString("book.bookId ASC");
    }
}
